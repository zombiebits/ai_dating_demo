###############################################################################
# BONDIGO – username + password Supabase‑Auth demo
###############################################################################
...
def buy(user:dict, comp:dict):
    ...
    owned = SB.table("collection").select("companion_id") \
...
# ───────── AUTH BLOCK (username + pwd) ─────────
if "user" not in st.session_state:
    st.title("🔐 Login to **BONDIGO**")

    mode   = st.radio("Account", ["Sign in", "Sign up"], horizontal=True)
    uname  = st.text_input("Username", key="u")
    pwd    = st.text_input("Password", type="password", key="p")
    act    = st.button("Go ➜")

    if act and uname and pwd:
        pseudo_email = f"{uname}@bondigo.local"

        try:
            if mode == "Sign up":
                SB.auth.sign_up({"email": pseudo_email, "password": pwd})
            # sign‑in (works immediately after sign‑up, no e‑mail flow)
            sess = SB.auth.sign_in_with_password({"email": pseudo_email,
                                                  "password": pwd})
            # profile row
    ...
    if prompt:
        ...
        try:
            ...
            SB.table("messages").insert(
              {"user_id":user["id"],"companion_id":cid,"role":"user","content":prompt}).execute()
            SB.table("messages").insert(
              {"user_id":user["id"],"companion_id":cid,"role":"assistant","content":reply}).execute()
        except RateLimitError: st.warning("OpenAI rate‑limit.")
        except OpenAIError as e: st.error(str(e))

# ───────── COLLECTION ─────────
elif page=="My Collection":
    st.header("My BONDIGO Collection")
    if not colset: st.info("Nothing minted yet.")
